...
=== Colab setup (run these in a cell first) ===
    ...
    %cd repo/rfi/finetune-demo/dpo

    # Or upload the whole finetune-demo/ folder directly via the Colab file
    # browser (Files pane -> upload), then %cd into finetune-demo/dpo.

    # Make sure the SFT adapter (../sft/lora-adapter/) is present — either
    # trained earlier and uploaded alongside, or retrained here first with
    # ../sft/train_lora.py (same script works on Colab's GPU too, just faster).
